...
-- Generato il: 15 lug, 2018 at 07:14 PM
...
(1, 'CANON', 'TOMMI', 'tommasopirozzi@hotmail.it', 'TOMMASO', 'PIROZZI', 'PRZTMS92P04G309L', '3343942027', 6000, '1992-05-25'),
(2, 'MARC', 'TOMMI', 'marcello@gmail.com', 'MARCELLO', 'QUATTROMANI', 'MRCQTT95Q00T606L', '3354548484', 25000, '1995-12-10'),
(3, 'GIU', 'TOMMI', 'vanesio@live.com', 'GIULIANO', 'VANESIO', 'VNSGLN93D02F839M', '3405117062', 30000, '1993-04-02'),
(5, 'VinTor', 'vinc', 'vinc@hotmail.it', 'VINCENZO', 'TORINO', 'VINTOR92P04G301P', '0810000000', 3000, '1992-06-19'),
(6, 'Ross', 'okok', 'marior@gmail.com', 'Mario', 'Rossi', 'MAROSS92P04G301P', '3335696025', 5000, '1993-05-20'),
(7, 'D10S', 'maradona10', 'diego@hotmail.it', 'Diego', 'Maradona', 'DIGMAR92P04G301P', '3391010010', 2000, '1980-06-05'),
(8, 'Green', 'verde', 'verdi@live.it', 'Ciro', 'Verdi', 'VERCIR92P04G301P', '3405252633', 3500, '1985-10-12'),
(9, 'oooo', 'haaaa', 'ajs@fkf.it', 'WOW', 'SNIX', 'turnfh12h34g374y', '0815059922', 1500, '1992-07-15'),
(10, 'HANRY', 'proviamoquesta', 'hanry@ok.it', 'HANRY', 'RADRIZ', 'RYEIWO12I34U567H', '3323344887', 1250, '1993-07-02');
...
CREATE TRIGGER `addsicurezza_BEFORE_INSERT` BEFORE INSERT ON `addsicurezza`
 FOR EACH ROW BEGIN
	IF(NOT NEW.NAME REGEXP '^[a-zA-Z]+(['']?[a-zA-Z ]+)*$')
	THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Error : Invalid name format ';
	END IF;

	IF(NOT NEW.SURNAME REGEXP '^[a-zA-Z]+(['' -]?[a-zA-Z ]+)*$')
	THEN
	...
	IF(NOT NEW.EMAIL REGEXP '^[a-zA-Z0-9]{1}[a-zA-Z0-9._%-]{0,62}[a-zA-Z0-9_%-]{1}@[a-zA-Z0-9]{1}[a-zA-Z0-9._%-]{1,252}.[a-zA-Z]{2,4}$')
	THEN
...
(5, 'PROVA1', 'SUPER EVENTO', 'CINEMA', 'COMEDY', '2018-06-18', '80019', 'StadioQualiano'),
...
CREATE TRIGGER `evento_BEFORE_INSERT` BEFORE INSERT ON `evento`
 FOR EACH ROW BEGIN
	IF(NOT NEW.DESCRIPTION REGEXP '^[a-zA-Z0-9 
,.;'"/_#?!-]+$')
	THEN
	...
	IF(NOT NEW.PLACE_NAME REGEXP '^(([a-zA-Z]{1}[°]?)|([0-9]+[°]?))(([ ',."/-]{1}(([a-zA-Z]{1}[°]?)|([0-9]+[°]?)))*["']?[,./-]?[a-zA-Z 0-9]+)*[.]?$')
	THEN
		SIGNAL SQLSTATE '45009'
		SET MESSAGE_TEXT = 'Error : Invalid place name format ';
	END IF;
	IF(NOT NEW.TITLE REGEXP '^[a-zA-Z0-9 ,.;'"/_#?!-]+$')
	THEN
...
(5, 1, '17:00:00', '18:00:00', 12),
(5, 1, '17:00:00', '18:00:00', 13),
(9, 1, '17:00:00', '18:00:00', 14);

...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `gestione_turno_BEFORE_INSERT` BEFORE INSERT ON `gestione_turno` FOR EACH ROW BEGIN
	IF(END < START) THEN
		...
		SET MESSAGE_TEXT = 'Error : The END of turn date can''t be lower than the START turn date';
	END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `gestione_turno_BEFORE_UPDATE` BEFORE UPDATE ON `gestione_turno` FOR EACH ROW BEGIN
	IF(END < START) THEN
		SIGNAL SQLSTATE '45016'
		SET MESSAGE_TEXT = 'Error : The END of turn date can''t be lower than the START turn date';
	END IF;
...
-- Dump completed on 2018-07-07  0:37:16
